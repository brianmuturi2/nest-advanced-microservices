...
export class OutboxProcessor {
    ...
    constructor(
        ...
        private workflowService: ClientProxy,
    ...
    * because it provides more sophisticated features e.g locking, supports multiple nodes running in parallel
    */
   @Cron(CronExpression.EVERY_SECOND)
   async processOutboxMessages() {
    this.logger.debug(`${OutboxProcessor.name} Processing outbox messages`);
   ...
   async dispatchWorkflowEvent(outbox: Outbox) {
    ...
    await lastValueFrom(this.workflowService.emit(outbox.type, rmqRecord));
   }
}